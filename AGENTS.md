...
Any change to an `assume val`, an in-proof `assume`/`admit`, a `friend` declaration, the builder-generated `C.Loops` block in `scripts/flake/verify_fstar.sh`, or a provider/tool pin must update `spec/assumption-register.json` and keep `python3 scripts/validation/assumption_graph.py check` green on the `verify-fstar` evidence; cryptographic hardness is never stated as an F* axiom but as a named bad event with an external register premise (see `docs/verification/fstar/assumption-graph.md`).
...
Follow the description formats in [CONTRIBUTING.md](CONTRIBUTING.md#issue-and-pull-request-descriptions), including when posting through the CLI or API. PRs use `## Summary`, `## Validation`, `## Notes`; Issues use `## Summary`, `## Context`, `## Acceptance criteria`, `## Notes`, in that order. Keep substantive historical evidence and decisions intact when reformatting, and distinguish historical records from new execution. Back up existing bodies and check for concurrent edits before replacement. Use structured body arguments or a UTF-8 body file for multiline publication, and verify the published text and labels.

Keep repository documentation, logs, branch names, commit messages, PR titles, descriptions, and comments scoped to Aegaeon: its own responsibilities, protocol and API contracts, behavior changes, and validation. Do not introduce or assume a particular connecting application, downstream service architecture, deployment, or cross-repository test outcome as part of Aegaeon's specification or assurance. Mention a specific application or product name, or a particular connecting architecture, only when the user explicitly asks for it. A product used incidentally in testing does not grant that permission. Where an interface needs explanation, use the standard protocol role (such as OAuth client or resource server), describe only Aegaeon's side of the contract, and use reserved example domains. Check publication text and attached logs for unrelated application details before sharing. Do not silently alter API identifiers or hash-bound evidence: preserve originals privately, and publish generic descriptions or separately frozen redacted copies; API renaming requires a reviewed compatibility change.
