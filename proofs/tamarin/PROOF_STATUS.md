...
| stepup/stepup_acr_enforcement.spthy | Extended RFC 9470 ACR/max_age model. Not referenced by the current compliance-matrix claim set; kept in `ci/tamarin_proofs_extended.sh` and **not** CI-gated because the model is reachability-non-convergent under the blocking CI budget: every exists-trace witness (even a trivial `Access_Granted`) times out, so the fast-"verified" all-trace lemmas carry no machine-checked non-vacuity witness, and `max_age_triggers_reauth` does not converge under any tried heuristic. Gating awaits Session-provenance convergence work (`reuse`/`sources` helpers). See the detailed note under "stepup/stepup_acr_enforcement.spthy (RFC 9470, extended)". |
...
**Not CI-gated — reachability non-convergence (triage 2026-08-25).** A CI-faithful triage (tamarin 1.12.0; 600 s/lemma; derivcheck 180 s) found the all-trace lemmas `acr_level_enforcement`, `challenge_session_binding`, `no_acr_downgrade`, `step_up_preserves_identity`, and `challenge_single_use` verify in 0–1 s, but every exists-trace non-vacuity witness times out: the full step-up→mfa-access witness at ~309 s and even the trivial `Ex … Access_Granted(…)@g` at ~150 s. `max_age_triggers_reauth` does not converge under the default heuristic (~610 s) nor under `S`/`C`/`i`/`I` (≥240 s each). Precomputation is clean (`[Saturating Sources] Done`, no partial-deconstruction warnings), so the blow-up is in proof search: `Access_Granted` re-emits `Session` and several rules produce `Session`, so the backward Session-provenance search does not terminate without a `reuse`/`sources` helper lemma. Consequently the fast all-trace results are **not** backed by a machine-checked reachability witness and could be vacuous, so they are not gated — this is a deliberate, evidence-backed deferral, not a claim. Gating awaits Session-provenance convergence work. The RFC 9470 matrix claim is unaffected: it rests on `fstar/stepup/StepUp.fst` and the gated, reachability-witnessed `stepup/stepup_soundness.spthy`.
