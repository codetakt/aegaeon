...
Document revision: **2026-09-12-r1**.
...
The 2026-09-07-r3 revision pins each companion document revision, names critical Discovery
...
The 2026-09-12-r1 status revision distinguishes the historical solver-recording
mismatch from the corrected explicit pin. Selected proof evidence was replayed
with matching recorded and observed solver identities. This update does not
attest the solver, accept outstanding premises or activate the foundation claim.
...
| Axiom soundness | G-08, G-11 and dependents | Concrete hash injectivity, the vacuous Ed25519 statement and the different-key JWS assumption were removed (2026-09-11) and the affected proofs re-run with explicit bad-event premises; the effective dependency closure is reconstructed by `scripts/validation/assumption_graph.py`. The historical solver-identity mismatch is corrected by explicitly passing the pinned F* package's bundled Z3 4.13.3 through `FSTAR_SOLVER`/`--smt`; recorded and observed identities match in the corrected five-pass evidence. Still open: acceptance review of `spec/assumption-register.json` (every entry is `specified-not-attested`), solver soundness, the lax-loaded HACL\*/EverParse provider sources, the builder-injected `C.Loops` premises reached by the EverParse-derived parsers, and runtime correspondence of the hash/HMAC/Ed25519 adapters |
