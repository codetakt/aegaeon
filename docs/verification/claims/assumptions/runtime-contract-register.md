...
|---|---|---|---|---|---|
| RC-1 | **OS entropy quality.** Each 32-byte read from the OS CSPRNG (`getrandom`) is fresh, unpredictable output with full cryptographic strength. | Kernel CSPRNG via `getrandom` (`crates/crypto/src/rand.rs`) | Opaque 32-byte seed parameter; quality not modeled | Auth codes, tokens, challenges, and DPoP nonces become predictable; Tamarin freshness assumptions fail | Low |
...
| RC-3 | **HMAC-SHA256 is a PRF, and the runtime implements it correctly.** DRBG unpredictability and distinctness of derived values (challenge IDs, generated tokens) require HMAC-SHA256 to be a pseudorandom function and the `hmac`/`sha2` crates (`crates/crypto/src/mac.rs`) to implement HMAC correctly. | RustCrypto `hmac` + `sha2` crates (unverified) | F\* proves the SP 800-90A **construction** only (composition, lengths, counters), not PRF security | DRBG output predictable or colliding despite the verified construction | Low |
