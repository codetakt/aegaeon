...
1. **Cryptographic trust boundaries** — **0 assume vals.** Cryptographic
   ...
   register entries `A-SHA256-CR`, `A-SHA384-CR`, `A-SHA512-CR`,
   `A-SHA2-HALF-TRUNC-CR`, `A-HMAC-SHA2-EUF-CMA` and `A-ED25519-EUF-CMA` (§3.1–§3.4).
...
and `A-SHA2-HALF-TRUNC-CR` for all three `truncation_collision` instances:

| Hash algorithm | Retained output bits | Generic birthday work |
|---|---|---|
| SHA-256 | 128 | about 2^64 evaluations |
| SHA-384 | 192 | about 2^96 evaluations |
| SHA-512 | 256 | about 2^128 evaluations |

The graph binds events at module granularity, so the dispatcher carries all
three algorithm premises. `oidc_hash_collision` requires both the applicable
full-hash and truncation premises; one cannot substitute for the other.
These bounds specify external premises, not a quantitative security evaluation.
The truncation entry remains `specified-not-attested`. Its former identifier,
`A-SHA256-TRUNC128-CR`, described only one instance; historical graphs retain
that identifier and their original register hash and require regeneration.
...
| Status | Count | Notes |
|---|---|---|
| **Removed** (former computational hardness lemmas) | 6 | #1–#6; the premises are now `spec/assumption-register.json` entries `A-SHA256-CR`, `A-SHA384-CR`, `A-SHA512-CR`, `A-SHA2-HALF-TRUNC-CR`, `A-HMAC-SHA2-EUF-CMA`, `A-ED25519-EUF-CMA` (`specified-not-attested`) |
