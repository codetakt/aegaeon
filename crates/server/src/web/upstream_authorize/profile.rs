...
fn record_upstream_authorize_profile_rejection(reason: &'static str) {
...
fn validate_upstream_authorize_profile_policy(
    ...
    {
        record_upstream_authorize_profile_rejection("grant_type_disallowed");
    ...
    {
        record_upstream_authorize_profile_rejection("token_endpoint_auth_disallowed");
        return Err(json_error_with_iss(
            StatusCode::BAD_REQUEST,
            "invalid_request",
    ...
    if profile.sender_constrained != SenderConstraint::None {
        record_upstream_authorize_profile_rejection("sender_constraint_disallowed");
        return Err(json_error_with_iss(
            StatusCode::BAD_REQUEST,
            "invalid_request",
...
pub(super) async fn resolve_upstream_authorize_profile(
    state: &AppState,
    issuer_base: &str,
    ...
    .await
    {
        ...
        Err(oauth_profile::ProfileError::MissingProfile) => {
            record_upstream_authorize_profile_rejection("profile_missing");
        ...
        Err(oauth_profile::ProfileError::InvalidIssuer) => {
            record_upstream_authorize_profile_rejection("issuer_invalid");
            return Err(no_cache_json_error_with_iss(
        ...
        Err(oauth_profile::ProfileError::Database(_)) => {
            record_upstream_authorize_profile_rejection("lookup_failed");
            return Err(no_cache_json_error_with_iss(
                StatusCode::INTERNAL_SERVER_ERROR,
                "server_error",
    ...
    crate::metrics_integration::MetricsIntegration::with_global(|metrics| {
        metrics.record_oauth_profile_usage(OAUTH_PROFILE_TYPE_UPSTREAM, "upstream_authorize");
    });
    validate_upstream_authorize_profile_policy(&profile, issuer_base, auth_method)?;
    Ok(profile)
}
