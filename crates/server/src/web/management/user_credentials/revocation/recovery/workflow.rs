...
pub(super) async fn revoke_user_recovery_token_inner(
    ...
    .await?;
    let revoked = local_credentials::revoke_recovery_token(
        &mut tx,
        token_id,
        user_id,
    ...
    .await
    .map_err(|_| management_internal_error(request_id, "Failed to revoke recovery token"))?
    .ok_or_else(|| {
        error_response(
            StatusCode::NOT_FOUND,
            "not_found",
            "Active recovery token not found",
            None,
            Some(request_id),
        )
    })?;
    ...
    write_user_management_audit_event(
        ...
        request_id,
        EndUserAuditEvent {
            event_type: "management.user.recoveryToken.revoked.v1",
            target_id: user_id,
