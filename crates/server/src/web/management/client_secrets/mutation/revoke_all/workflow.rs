use super::super::super::super::{
    begin_management_transaction, client_not_found, commit_management_transaction,
    ensure_base_configuration_matches, environment_from_management_record,
    load_management_environment_record_for_update, management_internal_error,
    parse_team_environment_client_scope, require_team_lifecycle_role_in_transaction,
    RuntimeClientMutationSync,
};
use super::super::super::audit::write_all_client_secrets_revoked_audit;
use super::super::super::scope::prepare_client_secret_lifecycle_scope;
use super::super::super::store::revoke_all_client_secrets_rows;
use crate::management::types::{ConfigurationTransactionRequest, EnvironmentMutationResponse};
use crate::web::management::client_store::load_client_for_update;
use crate::web::management::state::ManagementSession;
use axum::response::Response;
use sqlx::PgPool;

pub(super) async fn revoke_all_client_secrets_inner(
    pool: &PgPool,
    ...
    let updated = revoke_all_client_secrets_rows(
        &mut tx,
        team_id,
        environment.scope.environment,
        client_id,
    ...
    .await?;
    if updated {
        write_all_client_secrets_revoked_audit(
            &mut tx,
            &environment,
