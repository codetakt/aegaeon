...
# Expected rejections remain outside module admission and the assumption graph.
# --hint_info records solver starts without --detail_errors, which expands an
# expected error into diagnostic Error 9 reports and changes fixture behavior.
run_control() {
	local name="$1"
	shift
	local recorder_log="$OUT_DIR/${name}-controls.recorder.log"
	sha256sum "$@" >"$OUT_DIR/${name}-controls.sha256"
	if ! python3 "$REPO_ROOT/scripts/validation/run_fstar_invocation.py" \
		--out-dir "$OUT_DIR/controls" --pass-id "$name" -- \
		fstar.exe --hint_info --smt "${FSTAR_SOLVER:-}" --include authcode "$@" \
		>"$recorder_log" 2>&1; then
		cat "$recorder_log" >&2
		echo "[FAIL] $name negative-control gate" | tee -a "$LOG" >&2
		return 1
	fi
	cp "$OUT_DIR/controls/invocations/$name/output.log" "$OUT_DIR/${name}-controls.log"
	if ! python3 "$REPO_ROOT/scripts/validation/check_fstar_controls.py" \
		--out-dir "$OUT_DIR" --control "$name"; then
		cat "$recorder_log" >&2
		echo "[FAIL] $name negative-control gate" | tee -a "$LOG" >&2
		return 1
	fi
	echo "[OK] $name negative-control gate (not admitted proof evidence)" | tee -a "$LOG" >&2
}

run_control redis-flag \
	authcode/AuthCode.RedisFlag.fst ../tests/fstar/property/TestAuthCodeRedisFlag.fst
run_control authorization-revision \
	authcode/Authorization.ProjectionRevision.fst ../tests/fstar/property/TestAuthorizationProjectionRevision.fst
python3 "$REPO_ROOT/scripts/validation/check_fstar_controls.py" \
	--out-dir "$OUT_DIR" --verify-records
