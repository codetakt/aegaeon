...
function usage() {
  return [
    "Usage:",
    "  node --experimental-strip-types scripts/sdk/download_admin_sdk_evidence.ts",
...
function findWorkspaceRoot(explicitRoot) {
  ...
  while (true) {
    if (
      ...
      && existsSync(
        path.join(current, "spec", EVIDENCE_FILE_NAME.replace(".json", ".schema.json")),
      )
    ) {
      return current;
    }
...
main().catch((error) => {
  console.error(
    "[download-admin-sdk-evidence] error:",
