...
function usage() {
  return [
    ...
    "  node --experimental-strip-types " +
      "scripts/sdk/build_sdk_released_client_claim_report.ts [options]",
    "",
    "Options:",
...
function parsePublicationOrgTasks(entries) {
...
function parseIsoTimestamp(value, label, blockers) {
...
function computeEvidenceAgeHours(timestamp, now, label, blockers) {
  if (!timestamp) {
    return null;
  }
...
async function main() {
  ...
  const [
    ...
    loadOptionalJson(managedProviderEvidencePath),
    loadOptionalJson(adminSdkEvidencePath),
    loadOptionalJson(promotionReportPath),
  ]);
  ...
  if (managedProviderEvidencePresent) {
  ...
  const managedProviderEvidenceGeneratedAt = managedProviderEvidencePresent
  ...
  const adminSdkEvidenceGeneratedAt = adminSdkEvidencePresent
