...
const DEFAULT_POLICY_PATH = path.join(MODULE_DIR, "sdk_release_custody.current.json");

function usage() {
  return [
    "Usage:",
    "  node --experimental-strip-types " +
      "scripts/sdk/check_sdk_release_custody.ts " +
...
function validatePolicy(policy) {
  if (!policy || typeof policy !== "object") {
    throw new Error("Release custody policy must be an object");
  }
  return {
...
function normalizeActual(actual) {
  if (!actual || typeof actual !== "object") {
    throw new Error("Release custody payload must be an object");
  }
...
function compareAgainstPolicy(policy, actual) {
  ...
  for (const variable of policy.requiredVariables) {
    ...
    if (variable.pattern && (value === null || !(new RegExp(variable.pattern).test(value)))) {
      mismatches.push(
...
async function main() {
  ...
  if (mismatches.length > 0) {
    console.error(
      `[release-custody] ${actualLabel} does not match ` +
  ...
  console.log(
    `[release-custody] ${actualLabel} matches ` +
      path.relative(process.cwd(), policyPath),
  );
}

main().catch((error) => {
  console.error("[release-custody] error:", error.message ?? error);
  process.exitCode = 1;
});
