...
function usage() {
  return [
    "Usage:",
    "  node --experimental-strip-types scripts/sdk/build_sdk_release_attestation.ts [options]",
    "",
    "Options:",
    ...
    "                                     .artifacts/release/release-attestation.public.pem",
    "  --signature-descriptor <path>     Default:",
...
function parseArgs(argv) {
  const options = {
    ...
    signatureDescriptor:
      ...
      ?? DEFAULT_SIGNATURE_DESCRIPTOR_PATH,
  };
...
async function readKeyMaterial() {
  ...
  throw new Error(
    "AEGAEON_SDK_SIGNED_RELEASE_ATTESTATION=true requires AEGAEON_COSIGN_KEY"
      + " or AEGAEON_COSIGN_KEY_PATH",
  );
}

function detectSignatureAlgorithm(privateKey) {
  switch (privateKey.asymmetricKeyType) {
    ...
    default:
      throw new Error(`Unsupported signing key type: ${privateKey.asymmetricKeyType}`);
  }
}
...
async function main() {
  ...
  const attestation = {
    ...
    source: {
      github_ref: process.env.GITHUB_REF ?? publishManifest.source?.githubRef ?? null,
      github_sha: process.env.GITHUB_SHA ?? publishManifest.source?.githubSha ?? null,
      github_run_id: process.env.GITHUB_RUN_ID ?? publishManifest.source?.githubRunId ?? null,
      github_workflow:
        process.env.GITHUB_WORKFLOW ?? publishManifest.source?.githubWorkflow ?? null,
      npm_dist_tag: process.env.AEGAEON_NPM_DIST_TAG ?? publishManifest.source?.npmDistTag ?? null,
    },
    publication: {
    ...
    verified_core: {
      ...
      handoff_manifest_path: publishManifest.verifiedCore?.handoffManifestPath ?? null,
      handoff_manifest_sha256: publishManifest.verifiedCore?.handoffManifestSha256 ?? null,
    },
  ...
  console.log(
    `[build-release-attestation] wrote ${path.relative(rootDir, signaturePath)}`
      + ` and ${path.relative(rootDir, signatureDescriptorPath)}`,
  );
}

main().catch((error) => {
  console.error(
    "[build-release-attestation] error:",
