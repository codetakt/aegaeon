...
type ClientClaimPromotionReport = {
  schema_version: number;
  generated_at: string;
  policy_path: string;
  claim_boundary_path: string;
  release_attestation_path: string;
  managed_provider_evidence_path: string;
  admin_sdk_evidence_path: string;
...
function usage(): string {
  return [
    "Usage:",
    "  node dist-tools/check-client-claim-promotion.js [options] " +
...
function findWorkspaceRoot(explicitRoot: string | null): string | null {
  if (explicitRoot) {
...
function parseLanes(entries: string[]): Map<string, string> {
...
async function main(): Promise<void> {
  ...
  const [
    ...
    readJson<ClientClaimPromotionPolicyFile>(policyPath),
    readJson<ClaimBoundaryFile>(claimBoundaryPath),
    readJson<ReleaseAttestationFile>(releaseAttestationPath),
    readJson<ManagedProviderEvidenceFile>(managedProviderEvidencePath),
    readJson<AdminSdkEvidenceFile>(adminSdkEvidencePath),
  ]);
  ...
  if (
    ...
    failures.push(
      "admin-console management SDK package mismatch: expected " +
        ...
        (adminSdkEvidence.sdk_boundary?.management_sdk_package ?? "missing"),
    );
  }
  ...
  const report: ClientClaimPromotionReport = {
    schema_version: 1,
    ...
    managed_provider_evidence_path: path.relative(
      rootDir,
      managedProviderEvidencePath,
    ),
