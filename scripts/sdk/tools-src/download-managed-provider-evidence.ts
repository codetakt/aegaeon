...
type DownloadManagedProviderEvidenceOptions = {
  root: string | null;
  repo: string | null;
  workflow: string;
  branch: string;
...
function usage(): string {
  return [
    "Usage:",
    "  node dist-tools/download-managed-provider-evidence.js --repo <owner/repo> [--workflow <workflow>] [--branch <branch>] [--artifact <name>] [--out <path>]",
    "  node dist-tools/download-managed-provider-evidence.js --artifact-dir <dir> [--out <path>]",
    "",
...
function parseArgs(argv: string[]): DownloadManagedProviderEvidenceOptions {
  const options: DownloadManagedProviderEvidenceOptions = {
    root: process.env.AEGAEON_SDK_ROOT ?? null,
    repo: process.env.AEGAEON_MANAGED_PROVIDER_EVIDENCE_REPOSITORY ?? process.env.GITHUB_REPOSITORY ?? null,
...
function findWorkspaceRoot(explicitRoot: string | null): string {
  ...
  while (true) {
    if (existsSync(path.join(current, "package.json")) && existsSync(path.join(current, "spec", "managed-provider-evidence.schema.json"))) {
      return current;
    }
...
async function downloadFromGitHub(
  rootDir: string,
  options: DownloadManagedProviderEvidenceOptions,
  outPath: string,
): Promise<void> {
  const repo = options.repo;
  if (!repo) {
    throw new Error("A repository is required to download managed-provider evidence from GitHub");
  }
  ...
  try {
    ...
    console.log(
      `[download-managed-provider-evidence] downloaded ${options.artifact} from ${options.repo}#${runId}`,
    );
    if (run.url) {
...
main().catch((error) => {
  console.error("[download-managed-provider-evidence] error:", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
