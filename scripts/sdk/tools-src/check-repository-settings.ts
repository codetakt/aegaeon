...
type RepositorySettingsOptions = {
  policy: string;
  actual: string | null;
  owner: string | null;
  repo: string | null;
...
function usage(): string {
  return [
    "Usage:",
    "  node dist-tools/check-repository-settings.js --owner <owner> --repo <repo> [--policy <path>]",
    "  node dist-tools/check-repository-settings.js --actual <repo-settings.json> [--policy <path>]",
    "",
    "Options:",
...
function parseArgs(argv: string[]): RepositorySettingsOptions {
  const options: RepositorySettingsOptions = {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    options[camelKey as keyof RepositorySettingsOptions] = value;
    index += 1;
  }
...
function validatePolicy(policy: unknown): RepositorySettingsPolicy {
  const record = ensureRecord(policy, "repository settings policy");
...
function normalizeActual(actual: unknown): RepositorySettingsActual {
  const record = ensureRecord(actual, "repository settings payload");
...
async function fetchRemoteActual(
  owner: string,
  repo: string,
  policy: RepositorySettingsPolicy,
): Promise<RepositorySettingsActual> {
...
function compareAgainstPolicy(policy: RepositorySettingsPolicy, actual: RepositorySettingsActual): string[] {
...
async function main(): Promise<void> {
  ...
  let actual: RepositorySettingsActual;
  ...
  if (mismatches.length > 0) {
    console.error(`[repository-settings] ${actualLabel} does not match ${path.relative(process.cwd(), policyPath)}`);
  ...
  console.log(`[repository-settings] ${actualLabel} matches ${path.relative(process.cwd(), policyPath)}`);
}

main().catch((error) => {
  console.error("[repository-settings] error:", error instanceof Error ? error.message : error);
  process.exitCode = 1;
});
