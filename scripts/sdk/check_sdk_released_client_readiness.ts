...
function usage() {
  return [
    ...
    "  node --experimental-strip-types " +
      "scripts/sdk/check_sdk_released_client_readiness.ts [options] " +
...
function resolveToolScript(...relativeCandidates) {
  for (const relativeCandidate of relativeCandidates) {
    const candidate = path.join(TOOL_ROOT, ...relativeCandidate);
    if (existsSync(candidate)) {
      return candidate;
    }
  }
  throw new Error(
    `Could not locate tool script: ` +
      relativeCandidates.map((candidate) => candidate.join("/")).join(", "),
  );
}
...
async function main() {
  ...
  await runStep(
    ...
    [
      resolveToolScript(
        ...
        ["dist-tools", "check-strict-types.js"],
      ),
      "--root",
      rootDir,
    ],
  ...
  const promotionArgs = [
    resolveToolScript(
      ["scripts", "sdk", "check_sdk_client_claim_promotion.ts"],
      ["dist-tools", "check-client-claim-promotion.js"],
    ),
    "--root",
    rootDir,
  ...
  const releasedClientArgs = [
    resolveToolScript(
      ["scripts", "sdk", "build_sdk_released_client_claim_report.ts"],
      ["dist-tools", "build-released-client-claim-report.js"],
    ),
    "--root",
    rootDir,
    "--managed-provider-evidence",
  ...
  const activationArgs = [
    resolveToolScript(
      ["scripts", "sdk", "check_sdk_released_client_claim_activation.ts"],
      ["dist-tools", "check-released-client-claim-activation.js"],
    ),
    "--root",
    rootDir,
    "--report",
  ...
  await runStep(
    ...
    [
      resolveToolScript(
        ["scripts", "sdk", "build_sdk_release_publication_bundle.ts"],
        ["dist-tools", "build-release-publication-bundle.js"],
      ),
      "--root",
      rootDir,
      "--managed-provider-evidence",
